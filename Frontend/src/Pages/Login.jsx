...
export default function Login() {
  ...
  const handleLogin = async (data) => {
    try {
      setIsLoading(true);
      const response = await fetch(`${VITE_BACKEND_URL}/api/auth/signin`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="flex justify-center items-center font-anta bg-gradient">
      <div className="md:w-1/3 w-1/2 md:h-3/4 h-1/3 md:mt-4 rounded-md p-6  card-gradient">
        ...
        <form
          className="space-y-6  gap-1 flex-wrap"
        ...
        w-3/4 ml-14
        rounded-full
         font-semibold hover:text-white py-2 px-4 border hover:border-transparent transition duration-500 outline-none flex-[1 0 4rem] ${
           isLoading
          ...
          <p className="text-white ml-56 my-1  text-xs italic">OR</p>
          <OAuth/>
          </div>
        </form>
